...
const MemberAccountHistory: React.FC<MemberAccountHistoryProps> = ({ accounts, memberId }) => {
  ...
  const memberAccountHistory = accounts
    ...
    .sort((a, b) => 
      new Date(b.latestActivation).getTime() - new Date(a.latestActivation).getTime()
    );
  ...
  if (memberAccountHistory.length === 0) {
    return (
      <Card>
        ...
        <CardContent>
          <p className="text-muted-foreground">Nenhuma conta utilizada ainda.</p>
        </CardContent>
      </Card>
    );
  }
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        {memberAccountHistory.map(account => (
