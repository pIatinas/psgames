...
const MemberCard: React.FC<MemberCardProps> = ({ member, className = '' }) => {
  ...
  const activeAccounts = member.accounts?.filter(slot => !slot.left_at)?.length || 0;
  ...
  return (
    <Link to={`/members/${member.id}`} className={`block ${className}`}>
      <Card className="overflow-hidden transition-all hover:-translate-y-1 hover:neon-purple-border rounded-lg">
