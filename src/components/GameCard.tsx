    ...
    <Card className="h-full hover:shadow-lg transition-shadow overflow-hidden cursor-pointer">
      <Link to={`/games/${generateGameSlug(game.id, game.name)}`} className="block h-full">
        <div className="aspect-square relative">
          ...
          <div className="absolute top-2 left-2 bg-black/70 text-white px-2 py-1 rounded text-sm font-semibold max-w-[calc(100%-1rem)]">
          ...
          <div className="absolute bottom-2 right-2 flex flex-wrap gap-1 justify-end">
            {game.platform.map((platform) => (
              <Badge key={platform} variant="secondary" className="text-xs bg-black/70 text-white border-0">
                {platform}
              </Badge>
            ))}
          </div>
