...
const AdminGames: React.FC<AdminGamesProps> = ({
  ...
  const handleEdit = (game: Game) => {
    ...
    const gameAccountIds = accounts.filter(account => account.games?.some(g => g.id === game.id)).map(account => account.id);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      if (success) {
        ...
        queryClient.setQueryData(['admin-games'], (oldData: Game[] | undefined) => oldData ? oldData.filter(game => game.id !== deleteGameId) : []);
        queryClient.invalidateQueries({
          queryKey: ['admin-games']
        });
        toast({
        ...
        <Table>
          ...
          <TableBody>
            ...
            return <TableRow key={game.id}>
                  ...
                  <TableCell>
                    {gameAccounts.length > 0 ? <div className="flex flex-wrap gap-1">
                        {gameAccounts.slice(0, 2).map(account => <Badge key={account.id} variant="outline" className="text-xs">
                            {account.email}
                          </Badge>)}
                        {gameAccounts.length > 2 && <Badge variant="outline" className="text-xs">
                  ...
                  {isAdmin && <TableCell className="text-right">
                      <div className="flex justify-end gap-2">
                        <Button variant="outline" size="sm" onClick={() => window.location.href = `/games/${game.id}`} className="hover:bg-white hover:text-gray-900">
                          Ver
                        </Button>
                        <Button variant="outline" size="sm" onClick={() => handleEdit(game)} className="hover:bg-white hover:text-gray-900">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Input id="image" type="url" value={formData.image} onChange={e => setFormData(prev => ({
                ...prev,
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input id="banner" type="url" value={formData.banner} onChange={e => setFormData(prev => ({
                ...prev,
              ...
              <div>
                ...
                <Input id="developer" value={formData.developer} onChange={e => setFormData(prev => ({
                ...prev,
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input id="genre" value={formData.genre} onChange={e => setFormData(prev => ({
                ...prev,
              ...
              <div>
                ...
                <Input id="release_date" type="date" value={formData.release_date} onChange={e => setFormData(prev => ({
                ...prev,
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {['PS5', 'PS4', 'PS3', 'VITA'].map(platform => <div key={platform} className="flex items-center space-x-2">
                    <Checkbox id={`platform-${platform}`} checked={formData.platform.includes(platform as GamePlatform)} onCheckedChange={checked => {
                  setFormData(prev => ({
                    ...prev,
                    platform: checked ? [...prev.platform, platform as GamePlatform] : prev.platform.filter(p => p !== platform)
                  }));
                }} />
            ...
            <div className="grid gap-2">
              ...
              <div className="max-h-48 overflow-y-auto border rounded p-2">
                {accounts.map(account => <div key={account.id} className="flex items-center space-x-2 p-1">
