...
const AccountCard: React.FC<AccountCardProps> = ({
  ...
  return <Card className={`h-full hover:shadow-lg transition-all cursor-pointer ${isFullyOccupied ? 'opacity-60' : ''}`}>
      <Link to={`/accounts/${generateAccountSlug(account.id, account.email)}`} className="block hover:scale-105 transition-all ">
        <CardContent className="p-4 py-3 flex flex-col h-full ">
          ...
          <div className="mt-auto space-y-2">
            <div className="flex gap-2">
              <div className={`flex-1 text-center p-2 rounded text-xs ${isSlotOccupied(1) ? 'bg-red-500/20 text-red-400 border border-red-500/30' : 'bg-green-500/20 text-green-400 border border-green-500/30'}`}>
                {isSlotOccupied(1) ? getSlotUser(1) : 'Livre'}
              </div>
              <div className={`flex-1 text-center p-2 rounded text-xs ${isSlotOccupied(2) ? 'bg-red-500/20 text-red-400 border border-red-500/30' : 'bg-green-500/20 text-green-400 border border-green-500/30'}`}>
